...
#### 议题
+ 数据EDA说明
	+ bureau数据存在多条，及相当于明细记录
	+ credit_card\_balance同一时刻存在两个active的账户
	+ credit_card\_balance -installments_payments用户每次还款行为都会产生一条记录，每期应还还应该是由客户自己选择确定的，即期数大的还款日可能早于期数小的，并且间隔不一定。
+ 变量构造基本约定
	+ 时间间隔：最近一年按季度划分，距离第二年按半年划分，两年以前整年划分；
	+ 构造类别：RFM变量 转移矩阵变量；根据变量覆盖度来决定实际的使用维度的程度；
	+ 待讨论
		+ 如何使用时序数据
		+ 需要确定逾期程度的划分标准
+ 下次讨论时间：下个周末
